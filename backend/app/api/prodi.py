...
def create_prodi(payload: ProgramStudiCreate, db: Session = Depends(get_db), _: User = Depends(require_roles(UserRole.admin, UserRole.kaprodi))):
...
def update_prodi(prodi_id: int, payload: ProgramStudiUpdate, db: Session = Depends(get_db), _: User = Depends(require_roles(UserRole.admin, UserRole.kaprodi))):
...
def delete_prodi(prodi_id: int, db: Session = Depends(get_db), _: User = Depends(require_roles(UserRole.admin))):
    row = db.query(ProgramStudi).filter(ProgramStudi.id == prodi_id).first()
    if not row:
        raise HTTPException(status_code=404, detail="Program studi tidak ditemukan")
    db.delete(row)
