...
def create_mata_kuliah(payload: MataKuliahCreate, db: Session = Depends(get_db), _: User = Depends(require_roles(UserRole.admin_prodi, UserRole.kaprodi))):
...
def update_mata_kuliah(mata_kuliah_id: int, payload: MataKuliahUpdate, db: Session = Depends(get_db), _: User = Depends(require_roles(UserRole.admin_prodi, UserRole.kaprodi))):
    data = db.query(MataKuliah).filter(MataKuliah.id == mata_kuliah_id).first()
    if not data:
        raise HTTPException(status_code=404, detail="Mata kuliah tidak ditemukan")
